...
router.post("/GetPathologies", (req, res, next) => {
  pool.getConnection((err, connection) => {
    if (err) throw err;
    console.log("connected as id " + connection.threadId);
    connection.query(
      ...
      (err, rows) => {
        ...
        if (rows.length === 1) {
          ...
          // Now connected and we have the user ID so we do the insert
          connection.query(
            "SELECT * FROM hasPathologies WHERE userId=?",
            userId,
            (err, rows) => {
              if (err) throw err;
              res.status(201).json({ success: true, data: rows });
...
router.post("/RemoveRelative", (req, res, next) => {
  pool.getConnection((err, connection) => {
    if (err) throw err;
    console.log("connected as id " + connection.threadId);
    connection.query(
    ...
    connection.query(
      ...
      (err, rows) => {
        ...
        if (rows.length === 1) {
          ...
          // Now connected and we have the user ID so we do the insert
          connection.query(
            "DELETE FROM hasSubscribed WHERE  userId=? AND patientId=?",
